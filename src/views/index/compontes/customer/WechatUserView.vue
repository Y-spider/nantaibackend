  ...
  <div class="content-box">
    ...
    <div class="table-box">
      <el-table :data="tableData" style="width: 100%">
        <el-table-column label="用户id" width="260">
        ...
        <el-table-column label="发帖上限" width="140">
          <template slot-scope="scope">
            <p>{{ scope.row.totalPost }}</p>
          </template>
        </el-table-column>
        <el-table-column label="今日剩余发帖数" width="140">
          <template slot-scope="scope">
    ...
    <el-dialog :title="form.openid" :visible.sync="dialogFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitMofifyUserInfo"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
export default {
  async created() {
    let res = await getWechatUserAPI(
      this.pageIndex,
      this.pageSize,
      this.search
    );
  ...
  methods: {
    ...
    handleCancel(flag) {
      if (flag == true) {
        ...
        this.$set(
          this.tableData,
          this.nowTableDataIndex,
          this.copyNowTableData
        );
      }
    ...
    async submitMofifyUserInfo() {
      ...
      let res = await modifyWechatUserInfoAPI(
        this.tableData[this.nowTableDataIndex]
      );
      if (!res) {
        // 说明更新失败
        ...
        this.tableData[this.nowTableDataIndex].avatar =
          this.copyNowTableData.avatar;
        this.dialogFormVisible = false;
      }
    },
    ...
    handleDelete(index, userData) {
      let status = userData.status == 0 ? 1 : 0;
      const modifyData = {
        openid: userData.openid,
        status: status,
      };
      modifyWechatUserInfoAPI(modifyData).then((res) => {
